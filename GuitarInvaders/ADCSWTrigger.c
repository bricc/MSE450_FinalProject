...
unsigned long Slide_Potentiometer(void);
...
void ADC0_InitSWTriggerSeq3_Ch1(void){ volatile unsigned long delay;
  ...
  delay = SYSCTL_RCGC2_R;         //    allow time for clock to stabilize
	...
	GPIO_PORTE_DIR_R &= ~0x07;        // PE0 & PE2 inputs (PE2: Slider, PE0: Right fire)
  ...
  GPIO_PORTE_AFSEL_R &= ~0x01;      // disable alt funct on PE0
	...
	// PE1 - Start Game/End Game 
	GPIO_PORTE_AMSEL_R &= ~0x02;      // disable analog on PE0
  GPIO_PORTE_AFSEL_R &= ~0x02;      // disable alt funct on PE0
	GPIO_PORTE_DEN_R |= 0x02;         // enable digital I/O on PE0
